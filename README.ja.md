...
3. クォート内の文字列を除去し、`grep "npm" package.json` のような誤検出を防止
4. 単語境界を考慮した正規表現で許可されていないパッケージマネージャーの呼び出しをチェック（`pnpm-lock.yaml` や `.npm/` などの誤検出を防止）
5. 違反があれば `deny` でブロック、なければそのまま実行を許可

パッケージマネージャーを検出できない場合は、警告を出してコマンドの実行を許可します。

> [!NOTE]
> クォート内の文字列を除去してからチェックするため、`bash -c "npm install"` のようなコマンドはブロック**されません**。これは既知のトレードオフです — 大半のケースでパッケージマネージャーのコマンドはクォートの外に記述されます。
